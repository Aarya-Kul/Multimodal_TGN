...
Saves three files:
data/ml_<name>.csv → processed edges (u, i, ts, label, idx)
data/ml_<name>.npy → edge feature matrix (one row per edge, plus a dummy zero at index 0) --> (num_edges + 1, edge_feature_dim)
data/ml_<name>_node.npy → node feature matrix, but since the example datasets don’t have node features, they just create: `rand_feat = np.zeros((max_idx + 1, 172))`
...
Output:
- ./data/ml_movielens_mm.csv: u,i,ts,label,idx
- ./data/ml_movielens_mm.npy: shape of (num_edges + 1, 1) and each edge feature = [rating]
- ./data/ml_movielens_mm_node.npy: shape of (max_node_id + 1, embed_dim)
...
Outputs (for dataset-name = "movielens_mm"):
  - ./data/ml_movielens_mm.csv
      columns: u,i,ts,label,idx
  - ./data/ml_movielens_mm.npy
      shape: (num_edges + 1, 1)         # edge feature = [rating]
  - ./data/ml_movielens_mm_node.npy
